...
public:
    ...
    void addBook(Book b)
    {
        ...
        if(count==size)
        {
            cout<<"Books has reached its maximum size."<<endl;
            return;
        }
        books[count]=b;
        count ++;
    ...
    {
        /*Remove the book with the given isbn from the list. After removing a book all the books below the removed book will be shifted up,
    ...
    BookShop mergeShop(BookShop b)
    {
        ...
        BookShop temp("MergedShop",this->size+b.size);
        temp.addBook(books,this->count);
        temp.addBook(b.books,b.count);
        return temp;
    }
};
